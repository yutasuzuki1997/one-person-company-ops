...
/**
 * エージェントの階層レベルを返す
 * 0: 秘書(ジェニー) / 1: 事業部長(DH) / 2: PM / 3: 担当者(specialist)
 * agents.json に `hierarchyLevel` が明示されていればそれを優先し、
 * 無ければ role から推定する
 */
function getAgentHierarchyLevel(agent) {
  if (!agent) return 3;
  if (Number.isInteger(agent.hierarchyLevel)) return agent.hierarchyLevel;
  const role = String(agent.role || '');
  if (/秘書|ジェニー|secretary/i.test(role) && !/個人プロジェクト秘書/.test(role)) return 0;
  if (/事業部長|DH|division.?head/i.test(role)) return 1;
  if (/PdM補佐|PdM|プロダクトマネージャ/i.test(role)) return 1; // 横断＝DH 相当
  if (/PM補佐|プロジェクトマネージャ|^PM$/i.test(role)) return 2;
  return 3;
}

/**
 * 与えられた agent の上長（事業部長）を agents 一覧から推定する。
 * 同じ project / 類似 project を持つ上位レベルのエージェントを返す。
 */
function getDivisionHeadForAgent(agent, agents) {
  if (!agent || !Array.isArray(agents)) return null;
  const myLevel = getAgentHierarchyLevel(agent);
  if (myLevel <= 1) return null;
  const myProject = String(agent.project || '');
  const candidates = agents.filter((a) => {
    if (a.id === agent.id) return false;
    const lv = getAgentHierarchyLevel(a);
    return lv < myLevel && lv <= 1;
  });
  if (candidates.length === 0) return null;
  // 完全一致 project > project prefix 共有 > 最上位
  const exact = candidates.find((c) => String(c.project || '') === myProject);
  if (exact) return exact;
  const prefix = candidates.find((c) => {
    const p = String(c.project || '');
    return p && (myProject.includes(p) || p.includes(myProject));
  });
...
module.exports = {
  CompanyRegistry,
  slugify,
  defaultAgentsForNewCompany,
  getAgentHierarchyLevel,
  getDivisionHeadForAgent,
};
