...
// ── メモリキャッシュ ─────────────────────────────────────────────────────
let memoryCache = { content: null, cachedAt: 0, cacheKey: null };
...
// ── 記憶コンテキストの取得（キャッシュ付き・圧縮対応） ─────────────────
// opts.full=true: 朝ブリーフィング等フル情報が要るとき（~4000字）
// 既定: 各ファイル 800字に圧縮（token節約）
async function getMemoryContext(token, owner = 'yutasuzuki1997', repo = 'Workspace', opts = {}) {
  if (!token) return '';

  const full = !!opts.full;
  const cacheKey = full ? 'full' : 'compact';

  // キャッシュ有効ならキャッシュを返す
  if (memoryCache.content && memoryCache.cacheKey === cacheKey && Date.now() - memoryCache.cachedAt < CACHE_TTL) {
    return memoryCache.content;
  }

  let context = '\n\n## 記憶・プロジェクト情報\n';
  const memoryFiles = ['memory/projects.md', 'memory/yuta-preferences.md'];
  const limit = full ? 2000 : 800;

  for (const filePath of memoryFiles) {
    const content = await loadFileFromWorkspace(filePath, token, owner, repo);
    if (content) {
      const truncated = content.length > limit ? content.slice(0, limit) + '\n…(以下省略: full=trueで全文)' : content;
      context += `\n### ${filePath}\n${truncated}\n`;
    }
  }

  memoryCache = { content: context, cachedAt: Date.now(), cacheKey };
  return context;
}
...
async function saveCompletionToWorkspace(task, summary, agentName, token) {
  ...
  // スキル側（_common.md）の約束と一致させるため reports/ 配下に保存する
  const filePath = `reports/${date}-${projectName}-${safeName}.md`;
