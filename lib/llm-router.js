...
// fetch は Node 18+ / Electron のグローバルを使用する。
// 旧 node-fetch(v3)は ESM 専用で require() すると ERR_REQUIRE_ESM になり、
// このモジュール全体(タスク分類・ルーティング・フォールバック)がロード時に落ちていた。
...
const ROUTING = {
  ...
  secretary:  { provider: 'anthropic', model: 'claude-sonnet-4-6' },
...
function selectModel(taskType) {
  return ROUTING[taskType] || { provider: 'anthropic', model: 'claude-sonnet-4-6' };
}
