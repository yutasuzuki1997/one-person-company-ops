...
class AgentTeamsManager {
  ...
  async startJenny() {
    ...
    this._restartAttempts = 0
    this.jennyReady = false // online=応答可能 は init(セッション確立)で確定する。spawn直後はまだ未準備。
    console.log('[agent-teams] ジェニープロセス起動 (init待ち / ネイティブAgent委託モード)')

    // 重要(デッドロック回避): claude(-p, stream-json入力)は最初のstdinメッセージを受け取るまで
    // system/init を出さない。自走ループは isJennyOnline()(=init受信)を送信前ゲートに使うため、
    // 何も送らないと「init来ない→online false→送信しない→init来ない」で永久に立ち上がらない。
    // 起動直後に軽量なプライミングを送ってセッションを確立(init誘発)する。
    setTimeout(() => {
      try {
        if (this.jennyProcess && this.jennyProcess.stdin.writable && !this.jennyReady) {
          this.jennyProcess.stdin.write(JSON.stringify({
            type: 'user',
            message: { role: 'user', content: '（システム起動の確認）あなたはオンラインです。今は何も実行・委託・調査せず、「待機中」とだけ短く返答してください。次の指示はこの後に届きます。' }
          }) + '\n')
          console.log('[agent-teams] ジェニーにプライミング送信 (init誘発)')
        }
      } catch (e) { console.error('[agent-teams] プライミング送信失敗:', e.message) }
    }, 500)
  }
  ...
  processJennyEvent(event) {
    ...
    // セッション確立 = ジェニーが応答可能(ready)になった瞬間。ここで初めて online を確定する。
    if (event.type === 'system' && event.subtype === 'init') {
      this.sessionId = event.session_id || this.sessionId
      if (!this.jennyReady) {
        this.jennyReady = true
        this.broadcast({ type: 'jenny_online' })
        console.log('[agent-teams] ジェニー応答可能 (init受信)')
      }
      return
    }
  ...
  isJennyOnline() {
    // online=応答可能。プロセス生存だけでなく init(セッション確立)受信済みを要件にする。
    return this.jennyProcess !== null && !this.jennyProcess.killed && this.jennyReady === true
  }
