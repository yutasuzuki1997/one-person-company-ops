...
/**
 * 配下候補ロスターを1行ずつ整形して返す。
 * レベル差がある（自分より下）かつ同じ／類似プロジェクトのみ抽出。
 */
function buildRoster(agent, allAgents) {
  if (!agent || !Array.isArray(allAgents) || allAgents.length === 0) return '';
  const myLevel = getAgentHierarchyLevel(agent);
  const myProject = String(agent.project || '');
  const lines = [];
  for (const a of allAgents) {
    if (a.id === agent.id) continue;
    const lv = getAgentHierarchyLevel(a);
    if (lv <= myLevel) continue; // 同格以上は委託対象外
    const p = String(a.project || '');
    const sameProject = p && myProject && (p === myProject || p.includes(myProject) || myProject.includes(p));
    const crossProjectByRole = myLevel <= 1; // 事業部長/秘書は横断委託OK
    // PMは「横断スペシャリスト」（エンジニア・SNS・リサーチ等）には投げられる
    const isCrossFunctionalSpecialist = /エンジニア|リサーチ|SNS|デザイナ|marketer|researcher|designer/i.test(String(a.project || a.role || ''));
    if (!sameProject && !crossProjectByRole && !isCrossFunctionalSpecialist) continue;
    lines.push(`- \`${a.id}\` : ${a.displayName || a.name} / ${a.role} / ${a.project || '—'}`);
  }
  if (lines.length === 0) return '';
  return `\n## 委託可能なエージェント（DELEGATE で agentId にそのまま貼る）\n${lines.join('\n')}\n`;
}

function buildAgentSystemPrompt(agent, skillsDir, allAgents = []) {
...
- 自分でできないことのみPMにエスカレーション`;
  ...
  const roster = buildRoster(agent, allAgents);
...
class AgentExecutor {
  ...
  async execute(agent, task, taskId, depth = 0) {
    ...
    const system = buildAgentSystemPrompt(agent, this.skillsDir, this.agents);
    if (depth > 0) {
      console.log(`[mailbox] → ${agent.displayName || agent.name} (${agent.id}) にタスク受信: ${task.slice(0, 60)}`);
    }
    ...
    // リサーチ系エージェントはweb searchツールを使う
    const isResearcher = /リサーチ|調査|research|アナリスト|analyst/i.test(String(agent.role || '') + String(agent.id || ''));
    let fullResponse = '';
    try {
      if (isResearcher && this.apiKey) {
        const result = await completeWithTools({
          apiKey: this.apiKey, model: this.model, system,
...
不要な場合は「了解、記録しました」のみ返答してください。`;
      ...
      console.log(`[mailbox] ↑ ${agent.displayName || agent.name} → ${divisionHead.displayName || divisionHead.name} に完了報告`);
